...
class Cohort:
    ...
    def get_5_yr_survival(self):
        """:returns the percentage of patients that survived beyond 5 years """

        self._number_surviving_5_years = 0

        #define total number of patients
        self.get_initial_pop_size()

        #define total number of pts alive after 5 years
        for obs in self.get_survival_times():
            if obs >= 5:
                self._number_surviving_5_years += 1

        #divide alive/total
        self._5_yr_survival = self._number_surviving_5_years / self.get_initial_pop_size()

        #return the result
        return self._5_yr_survival
...
class CohortOutcomes:
    ...
    def get_5_yr_survival(self):
        ...
        self._simulatedCohort._number_surviving_5_years = 0

        #define total number of patients
        self._simulatedCohort.get_initial_pop_size()

        #define total number of pts alive after 5 years
        for obs in self._simulatedCohort.get_survival_times():
            if obs >= 5:
                self._simulatedCohort._number_surviving_5_years += 1

        #divide alive/total
        self._simulatedCohort._5_yr_survival = self._simulatedCohort._number_surviving_5_years / self._simulatedCohort.get_initial_pop_size()

        #return the result
        return self._simulatedCohort._5_yr_survival
...
class MultiCohort:
    ...
    def __init__(self, ids, pop_sizes, mortality_probs):
        ...
        self._sumStat_mean_5yrSurvival = None
    ...
    def simulate(self, n_time_steps):
        ...
        for i in range(len(self._ids)):
            ...
            # store the 5 yr survival rate from this cohort
            self._5yrSurvival.append(cohort.get_5_yr_survival())
    ...
    def get_cohort_5_yr_survival(self, cohort_index):
        """ returns the 5 year survival % of an specified cohort
        :param cohort_index: integer over [0, 1, ...] corresponding to the 1st, 2ndm ... simulated cohort
        """
        return self._5yrSurvival[cohort_index]
